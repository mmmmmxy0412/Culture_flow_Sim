...
from agentverse.llms import llm_registry#自动执行agentverse.llms下的__init__.py文件，在初始化文件中自动实例化了一个Registry类

from agentverse.agents import agent_registry#同上
from agentverse.environments import BaseEnvironment, env_registry#同上
from agentverse.memory import memory_registry#同上
from agentverse.memory_manipulator import memory_manipulator_registry#同上

from agentverse.output_parser import output_parser_registry#同上
from agentverse.twitter_page import page_registry
from agentverse.info_box import info_registry
from agentverse.abm_model import abm_registry#同上
import mesa
...
def load_llm(llm_config: Dict):
    ...
    return llm_registry.build(llm_type, **llm_config)#第一个参数是模型名称：如 "deepseek-chat" 或 "text-davinci-003" **llm_config 把剩下的配置字典拆开成关键字参数传进去
...
def prepare_task_config(task, tasks_dir):
    ...
    #修改了task_config中的内容 是将原来config中的内容实例化后又重新填回了task_config中
    for i, agent_configs in enumerate(task_config["agents"]):
        llm = load_llm(agent_configs.get("llm", "text-davinci-003")) #agent_configs.get("llm", "text-davinci-003") 如果 "llm" 不存在，则返回默认 "text-davinci-003" 字符串。
        agent_configs["llm"] = llm
        agent_configs["memory"] = load_memory(agent_configs.get("memory", {}), llm)#所有load函数load完之后返回的都是一个实例好的类的对象
