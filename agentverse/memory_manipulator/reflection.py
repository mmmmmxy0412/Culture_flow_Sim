...
(e.g., keeping silence) and 10 is \
extremely important (e.g., opinions on social events) \
, rate the likely importance of the \
following piece of memory. \
If you think it's too hard to rate it, you can give an inaccurate assessment. \
The content or people mentioned is not real. You can hypothesize any reasonable context. \
...
(e.g., keeping silence) and 10 is \
needing quick attention or immediate response(e.g., being required a reply or reaction by others), rate the likely immediacy of the \
following statement. \
If you think it's too hard to rate it, you can give an inaccurate assessment. \
...
example format: insight (because of Evidence 1, 5, 3)"""

CONTEXT_QUESTION_PROMPT = """Given only the information above, what are 3 most salient high-level questions we can ask about your memory to deduce your possible response to the context?"""
...
class Reflection(BaseMemoryManipulator):
    ...
    def manipulate_memory(self) -> None:
        ...
        if self.should_reflect():
            logger.debug(
                f"Agent {self.agent.name} is now doing reflection since accumulated_importance={self.accumulated_importance} > reflection_threshold={self.importance_threshold // 2}"
            )
    ...
    def get_accumulated_importance(self):
        ...
        if memories_to_evaluate:
            self._evaluate_memories_batch_sync(memories_to_evaluate)
    ...
    def _evaluate_memories_batch_sync(self, memories):
        """
        同步批量评估多个 memory 的重要性和紧迫性
        使用多线程来加速 LLM 调用
        """
        from concurrent.futures import ThreadPoolExecutor, as_completed

        def evaluate_one(memory):
            importance = self.get_importance(memory.content)
            immediacy = self.get_immediacy(memory.content)
            return memory.content, importance, immediacy

        # 使用线程池并行执行 LLM 调用
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = {executor.submit(evaluate_one, memory): memory for memory in memories}

            for future in as_completed(futures):
                try:
                    content, importance, immediacy = future.result()
                    self.memory2importance[content] = importance
                    self.memory2immediacy[content] = immediacy
                    logger.debug(
                        f"Evaluated memory: {content[:50]}... (importance={importance}, immediacy={immediacy})")
    ...
    def get_context_questions(self, texts):
    ...
    def get_insights(self, statements):
        ...
        while 'Insight' in insights: insights.remove('Insight')
        while 'insight' in insights: insights.remove('insight')
        while '' in insights: insights.remove('')
        return insights
    ...
    def query_similarity(
        ...
        maximum_score = None
        for text in texts:
            embedding = get_embedding(self.agent.llm, text)
            score = []

            # 批量评估缺失的 memory 重要性和紧迫性
            memories_to_evaluate = []
            ...
            for memory in memory_bank:
                if memory.content not in self.memory2time:
                    self.memory2time[memory.content]["last_access_time"] = self.agent.current_time
                    self.memory2time[memory.content]["create_time"] = self.agent.current_time
    ...
    def get_memories_of_interest_all(self):
        memories_of_interest = []
        # for memory in self.memory.messages[-100:]:
        for memory in self.memory.messages[-20:]:
            memories_of_interest.append(memory)
        return memories_of_interest

    def memories_of_interest_to_string(self, memories_of_interest):
        res = []
        for m in memories_of_interest:
            if m.sender == self.agent.name:
                res.append(m.content)
            else:
                res.append(f"[{m.sender}]: {m.content}")
        res = list(set(res))
        return res
    ...
    def reflect(self):
        ...
        memories_of_interest = self.get_memories_of_interest_all()
        # questions = self.get_questions([m.content for m in memories_of_interest])
        questions = self.get_questions(self.memories_of_interest_to_string(memories_of_interest))
        statements = self.query_similarity(
            questions, len(questions) * 3, memories_of_interest, self.agent.current_time
        )
    ...
    def retrieve(self, questions, top_k=10):
        """
        retrieve the memory according to current perception to the environment
        """
        memories_of_interest = self.get_memories_of_interest_all()
        # questions = self.get_context_questions(context)
        statements = self.query_similarity(
            questions, top_k, memories_of_interest, self.agent.current_time
        )
        return '\n'.join(statements)